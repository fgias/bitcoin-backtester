...
class MarketDataSource:
    ...
    def start_market_simulation(self):
        ...
        for time, row in data.iterrows():
            self.md.add_last_price(time, "XBTUSD", row["Close"], row["Volume"])
            self.md.add_open_price(time, "XBTUSD", row["Open"])
...
import pandas as pd
class Backtester:
    def __init__(self, symbol, start_date, end_date, data_source): 
...
backtester = Backtester("XBTUSD", dt.datetime(2018, 1, 1), 
                        dt.datetime(2018, 12, 31), "csvfile")
