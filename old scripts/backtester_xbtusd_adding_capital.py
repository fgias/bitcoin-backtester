...
class MeanRevertingStrategy(Strategy):
    def __init__(self, symbol, lookback_intervals = 20, buy_threshold = -1.5, 
                 sell_threshold=1.5, cash=100000):
    ...
    def event_tick(self, market_data):
        ...
        sizing = self.calculate_size_of_position(market_data)

        if signal_value < self.buy_threshold:
            self.on_buy_signal(timestamp, sizing)
        elif signal_value > self.sell_threshold:
            self.on_sell_signal(timestamp, sizing)
    ...
    def calculate_size_of_position(self, market_data):
        self.prices = self.prices[-self.lookback_intervals:] 
        returns = self.prices["close"].pct_change().dropna() 
        kelly_frac = returns/(4 * (returns.std()) ** 2)
        sizing = numpy.floor((kelly_frac * self.cash)/market_data.get_last_price(self.symbol))
        return abs(sizing)    


    def on_buy_signal(self, timestamp, sizing): 
        if not self.is_long:
            self.send_market_order(self.symbol, sizing, True, timestamp)

    def on_sell_signal(self, timestamp, sizing): 
        if not self.is_short:
            self.send_market_order(self.symbol, sizing, False, timestamp)
...
backtester = Backtester("XBTUSD", dt.datetime(2017, 7, 20), 
                        dt.datetime(2019, 1, 1), data_source="csvfile")
